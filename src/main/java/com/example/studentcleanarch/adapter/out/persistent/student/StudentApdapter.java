...
public class StudentApdapter implements CreateStudent, UpdateStudent, GetStudent, DeleteStudent, SearchStudent, SortStudent {
    ...
    public List<Student> searchStudentByAddress(String address) {
        try {
            List<StudentJpaEntity> studentJpaEntityList = studentJpaRepository.findByAddress(address);
            for (StudentJpaEntity entity : studentJpaEntityList) {
                System.out.println(entity.getStudentId());
            }
            return studentJpaRepository.findByAddress(address).stream()
                    .map(StudentMapper::mapToDomainEntity)
                    .collect(Collectors.toList());
        } catch (Exception e) {
            throw new TimoException(500, "Student could not found address" + address);
        }
    }
    ...
    public List<StudentJpaEntity> sortStudentByBirthDateDesc() {
        return studentJpaRepository.findAll(Sort.by(Sort.Direction.DESC, "birthDate"));
    }

    @Override
    public List<StudentJpaEntity> sortStudentByBirthDateAsc() {
        return studentJpaRepository.findAll(Sort.by(Sort.DEFAULT_DIRECTION, "birthDate"));
    }
}
